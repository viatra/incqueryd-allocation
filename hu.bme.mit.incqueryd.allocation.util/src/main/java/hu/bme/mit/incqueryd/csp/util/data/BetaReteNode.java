...
public class BetaReteNode extends MemoryReteNode {
	...
	@Override
	public void print() {
		System.out.println("ReteNode: " + this.reteNode.getClass().getSimpleName() + " " + ReteProcessUtil.getJsonEObjectUri(this.reteNode) + " ,memory: " + memory);
		
		ReteNode left = leftParent.getParent();
		int leftTupels = leftParent.getTupleNumber();
		int leftArity = leftParent.getTupleArity();
		String leftID = left.getReteNode().getClass().getSimpleName() + " " + ReteProcessUtil.getJsonEObjectUri(left.getReteNode());
		...
		String rightID = right.getReteNode().getClass().getSimpleName() + " " + ReteProcessUtil.getJsonEObjectUri(right.getReteNode());
