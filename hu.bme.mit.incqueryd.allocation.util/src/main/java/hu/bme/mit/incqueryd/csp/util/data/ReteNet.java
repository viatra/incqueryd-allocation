...
public class ReteNet {
	...
	protected List<ReteNode> reteNodes = new ArrayList<ReteNode>();
	protected List<ReteEdge> reteEdges = new ArrayList<ReteEdge>();

	protected ReteRecipe recipe;
	protected Map<String, Long> inputStats;
	
	protected List<ReteProcess> processes = new ArrayList<ReteProcess>();
	private int[][] edges;
	...
	public List<ReteNodeRecipe> getReteNodeRecipesByProcessId(int processId){
		List<ReteNodeRecipe> recipes = new ArrayList<ReteNodeRecipe>();
	...
	private void createEdges() {
		...
		for (ReteNode node : reteNodes) {
			...
			if (reteNodeRecipe instanceof BetaRecipe) {
				...
				ReteNode leftParentNode = getReteNodeById(ReteProcessUtil.getJsonEObjectUri(leftParent));
				ReteNode rightParentNode = getReteNodeById(ReteProcessUtil.getJsonEObjectUri(rightParent));
				...
				ReteNode parentNode = getReteNodeById(ReteProcessUtil.getJsonEObjectUri(parent));
	...
	private void calculateHeuristicsInTheNet() {
		List<ReteNode> notReadyNodes = new ArrayList<ReteNode>();
		...
		do {
			List<ReteNode> readyNodes = new ArrayList<ReteNode>();
	...
	private void createProcesses() {
		List<ReteNode> remainingNodes = new ArrayList<ReteNode>();
		...
		do {
			List<ReteNode> assignedNodes = new ArrayList<ReteNode>();
	...
	private ReteNode getReteNodeById(String id) {
		for (ReteNode node : reteNodes) {
			if(ReteProcessUtil.getJsonEObjectUri(node.getReteNode()) == id) {
				return node;
			}
		}
