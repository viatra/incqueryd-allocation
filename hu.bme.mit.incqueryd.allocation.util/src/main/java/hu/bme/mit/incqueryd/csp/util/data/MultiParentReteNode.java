...
public class MultiParentReteNode extends ReteNode {
	...
	public MultiParentReteNode(ReteNodeRecipe reteNode) {
		...
		parentEdges = new ArrayList<ReteEdge>();
	}
	...
	@Override
	public void print() {
		System.out.println("ReteNode: " + this.reteNode.getClass().getSimpleName() + " " + ReteProcessUtil.getJsonEObjectUri(this.reteNode));
		
		for (ReteEdge parentEdge : parentEdges) {
			ReteNode parent = parentEdge.getParent();
			int tupels = parentEdge.getTupleNumber();
			int arity = parentEdge.getTupleArity();
			String parentID = parent.getReteNode().getClass().getSimpleName() + " " + ReteProcessUtil.getJsonEObjectUri(parent.getReteNode());
